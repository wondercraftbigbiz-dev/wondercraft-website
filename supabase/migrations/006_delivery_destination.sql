-- 006_delivery_destination
--
-- Stores where an order is actually going, in Econt's terms.
--
-- Until now `orders.contact_city` was a single free-text field, which cannot
-- hold a structured destination: Econt needs a city id (three different
-- Калояново exist, in three different regions) plus either an office/automat
-- code or a street address. `contact_city` is kept as a human-readable summary
-- so the admin list stays legible.
--
-- ── A LANDMINE, DELIBERATELY NOT ARMED ─────────────────────────────────────
-- Delivery is currently FREE, so the amount charged is unit_price × quantity and
-- `orders.total_eur` — a GENERATED column — is the whole truth. mark_order_paid
-- verifies the settled myPOS amount against it.
--
-- If delivery ever stops being free, adding a `shipping_eur` column here is NOT
-- enough and is actively dangerous: total_eur is generated from
-- unit_price_eur * quantity and cannot include shipping, so myPOS would charge
-- product+shipping while verification compared against product only, and EVERY
-- payment would be rejected as amount_mismatch.
--
-- The correct change is a migration that adds `shipping_eur` AND a
-- non-generated `grand_total_eur`, and repoints mark_order_paid at the latter.
-- ───────────────────────────────────────────────────────────────────────────
...
-- ---------------------------------------------------------------------------
-- place_order: same behaviour, plus the destination.
--
-- DROP + CREATE, not CREATE OR REPLACE: changing the argument list registers an
-- overload rather than replacing the function, which would leave two variants
...
-- plpgsql functions are granted EXECUTE to PUBLIC by default. Order intake must
-- only ever be reachable from server-side code.
...
-- ---------------------------------------------------------------------------
-- orders_admin: surface the destination next to the payment state.
-- ---------------------------------------------------------------------------
